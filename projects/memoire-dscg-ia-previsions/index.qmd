...
subtitle: "Un modèle XGBoost entraîné sur le journal de caisse pour transformer un prévisionnel statique en outil de pilotage opérationnel. Mémoire DSCG, 2023."
---
...
![Le chiffre d'affaires journalier de 2018 à 2023. Les deux paliers plats correspondent aux fermetures Covid ; le niveau grimpe nettement à partir de mi-2022.](img/fig-jeu-de-donnees.png){fig-alt="Série temporelle du chiffre d'affaires journalier de 2018 à 2023, montrant deux interruptions Covid et une hausse de niveau à partir de mi-2022."}
...
![L'Optimal Transport relève la période antérieure au 23 juillet (en gris) au niveau du régime récent (en teal), pour homogénéiser la série d'entraînement.](img/fig-optimal-transport.png){fig-alt="Graphique montrant les données originales en gris et les mêmes données transportées vers le haut en teal après Optimal Transport."}
...
![Zoom sur un mois : la structure hebdomadaire saute aux yeux, avec des pics réguliers de week-end. C'est exactement ce que les variables calendaires permettent au modèle de capter.](img/fig-saisonnalite-hebdo.png){fig-alt="Série du chiffre d'affaires sur un mois montrant une nette périodicité hebdomadaire, avec des pics le week-end."}
...
Sur les données de test, jamais montrées au modèle, le chiffre d'affaires journalier est prédit avec une **erreur moyenne d'environ 219 €** (MAE). La RMSE, qui pénalise davantage les gros écarts, monte à 290 € : signe que la plupart des jours sont bien prédits, mais que quelques-uns le sont franchement mal. Et comme les erreurs sont plus faibles à l'entraînement qu'au test, le modèle est **partiellement surajusté**, et je le diagnostique sans le masquer.

![Ventes réelles (en gris) contre prévisions du modèle XGBoost (en teal). La ligne verticale sépare les données d'entraînement (à gauche) des données de test (à droite).](img/fig-resultat-xgboost.png){fig-alt="Graphique comparant les ventes réelles en gris et les prévisions XGBoost en teal, avec une ligne verticale séparant entraînement et test."}
...
- **Surajustement partiel** : plus de données d'entraînement, variables externes, ou réingénierie des résidus.
...
::: {.travail-meta}
Réalisé dans le cadre du mémoire DSCG (UE7, 2023) : « Intégration de l'intelligence artificielle au sein de l'Expertise Comptable : vers une amélioration des prévisions financières », complété par une enquête auprès de 24 experts-comptables sur leur adoption de l'IA. Les figures sont des reconstructions aux couleurs du portfolio, fidèles aux tendances du modèle.
:::
